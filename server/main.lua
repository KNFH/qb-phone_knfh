...
AddEventHandler('qb-phone_new:server:TransferMoney', function(iban, amount)
    ...
    QBCore.Functions.ExecuteSql(false, "SELECT * FROM `players` WHERE `charinfo` LIKE '%"..iban.."%'", function(result)
        if result[1] ~= nil then
            ...
            TriggerClientEvent('QBCore:Notify', src, "رقم الحساب هذا غير موجود!", "error")
        end
    end)
end)
...
QBCore.Functions.CreateCallback('qb-phone_new:server:ScanPlate', function(source, cb, plate)
    ...
    if plate ~= nil then 
        ...
        TriggerClientEvent('QBCore:Notify', src, "لا توجد مركبة قريبة ..", "error")
        cb(nil)
    end
end)
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetCurrentTow', function(source, cb)
    local tow = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetCurrentMech', function(source, cb)
    local mech = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
...
QBCore.Functions.CreateCallback('qb-phone_new:server:GetCurrentTaxi', function(source, cb)
    local taxi = {}
    for k, v in pairs(QBCore.Functions.GetPlayers()) do
        local Player = QBCore.Functions.GetPlayer(v)
        if Player ~= nil then
